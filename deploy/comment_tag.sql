...
CREATE TABLE IF NOT EXISTS scrapetition.comment_tag (
...
CREATE TRIGGER tag_set_meta_on_update BEFORE UPDATE ON scrapetition.comment_tag
    FOR EACH ROW EXECUTE PROCEDURE scrapetition.set_meta_on_update();


CREATE TRIGGER adjust_privilege_on_insert BEFORE INSERT ON scrapetition.comment_tag
       FOR EACH ROW EXECUTE PROCEDURE scrapetition.adjust_privilege(436);

CREATE TRIGGER adjust_privilege_on_update BEFORE UPDATE ON scrapetition.comment_tag
       FOR EACH ROW EXECUTE PROCEDURE scrapetition.adjust_privilege(436);


ALTER TABLE scrapetition.comment_tag ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY assert_well_formed ON scrapetition.comment_tag
FOR INSERT TO scrapetitionuser
WITH CHECK (created_by = current_user);

CREATE POLICY assert_well_formed_null ON scrapetition.comment_tag
FOR INSERT TO scrapetitionuser
WITH CHECK (created_by is null); -- if null, then is set to
				 -- current_user by trigger.

CREATE POLICY allow_insert_to_scrapetitioneditor ON scrapetition.comment_tag
FOR INSERT TO scrapetitioneditor
WITH CHECK (true);


CREATE POLICY allow_select_to_creator ON scrapetition.comment_tag
FOR SELECT TO scrapetitionuser
USING (created_by = current_user);

CREATE POLICY allow_select_to_group_member ON scrapetition.comment_tag
FOR SELECT TO scrapetitionuser
USING ((privilege & 16) = 16
        AND pg_has_role(gid, 'MEMBER'));

CREATE POLICY allow_select_to_others ON scrapetition.comment_tag
FOR SELECT TO scrapetitionuser
USING (privilege & 2 = 2);

CREATE POLICY allow_select_to_scrapetitioneditor ON scrapetition.comment_tag
FOR SELECT TO scrapetitioneditor
USING (true);


CREATE POLICY allow_update_to_creator ON scrapetition.comment_tag
FOR UPDATE TO scrapetitionuser
USING (created_by = current_user);

CREATE POLICY allow_update_to_group_member ON scrapetition.comment_tag
FOR UPDATE TO scrapetitionuser
USING ((privilege & 16) = 16
        AND pg_has_role(gid, 'MEMBER'));

CREATE POLICY allow_update_to_others ON scrapetition.comment_tag
FOR UPDATE TO scrapetitionuser
USING (privilege & 2 = 2);

CREATE POLICY allow_update_to_scrapetitioneditor ON scrapetition.comment_tag
FOR UPDATE TO scrapetitioneditor
USING (true);

CREATE POLICY allow_delete_to_creator ON scrapetition.comment_tag
FOR DELETE TO scrapetitionuser
USING (created_by = current_user);

CREATE POLICY allow_delete_to_scrapetitioneditor ON scrapetition.comment_tag
