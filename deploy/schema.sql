...
--     row_seq record;
-- BEGIN
--     FOR row_seq IN SELECT sequence_name FROM information_schema.sequences
--     WHERE sequence_schema = 'public'
--     LOOP
--         EXECUTE 'ALTER SEQUENCE public.' || quote_ident(row_seq.sequence_name) || ' SET SCHEMA [new_schema];';
--     END LOOP;
-- END;
-- $$; 
...
-- $$
--     ALTER FUNCTION public.get_article_id_for_url(TEXT) SET SCHEMA scrapetition;
--     ALTER FUNCTION public.get_article(TEXT) SET SCHEMA scrapetition;
--     ALTER FUNCTION public.comments_insert(TEXT) SET SCHEMA scrapetition;
--     ALTER FUNCTION public.url_source_insert(TEXT) SET SCHEMA scrapetition;
-- $$;
